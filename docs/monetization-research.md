...
- **No email is sent today.** `users` holds `username`, `password_hash`, `role`,
  `status`, and no email address column exists (`src/server/store.ts:237`). Billing needs a
...
| €25 | The "unlimited" reading, meaning someone pointing a fleet or a load generator at it | Numeric caps published; fair-use clause; per-account cost tracking with alerts; manual suspension right (§10) | Nothing above it, because this is the ceiling. Upsell is annual billing, not a higher tier |
...
- **One refund** returns €1.00 to the customer while the €0.265 card fee is not returned
  as a percentage refund on Stripe's standard tier, so you are net negative on the customer.
...
| **Free tier, strict limits** | Low, since it is just an entitlement with small numbers | n/a | Low, **but not zero**: no payment contract, but §312(1a)/§327(3) BGB extend consumer-contract rules to "pay with data" contracts unless the data is processed *exclusively* to provide the service or meet legal requirements. This app's username/password-only free account fits that carve-out today, and adding analytics, marketing, profiling or nonessential telemetry would flip it (§5.11) **[FACT: [§312 BGB](https://www.gesetze-im-internet.de/bgb/__312.html), [§327 BGB](https://www.gesetze-im-internet.de/bgb/__327.html)]** | **Yes, this replaces the trial** |
...
| VAT | 19 % of gross, once registered | Not a cost, since it is not yours, but it is 19 % of the sticker price you never see **[FACT: §12 UStG]** |
...
because at 0.2 contacts/month more than half its contribution is gone, and **a single 10-minute
...
| Requests | 1M/day = 30M/mo | The real constraint | **[ASSUMPTION]** ~2,000 req/s achievable per CX32 for this workload, and this **must be measured**, see §2.6 |
...
(10 % of gross)**. The caps above are chosen to do that under the assumptions listed, and
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Available to German sole trader | Yes | Yes, GA, and Stripe markets it to German businesses **[FACT: [Stripe](https://stripe.com/managed-payments)]** | Yes | See §3.2 | Yes | Yes | Yes |
| Model | **Processor**, so you are the seller | **MOR**, so Link is merchant of record; statements read "Sold through Link" **[FACT: [docs](https://docs.stripe.com/payments/managed-payments/how-it-works)]** | **MOR** **[FACT]** | MOR | **Processor**, so you remain the seller **[UNVERIFIED: Mollie does not claim MOR status; confirm in Mollie's terms]** | **Processor** | **MOR** **[FACT: [Polar](https://polar.sh/resources/pricing)]** |
...
| VAT calculation | Stripe Tax: **0.5 %/transaction** (no-code) or €0.45/API call **[FACT]** | Included in the 3.5 % | Included | Included | **Not provided, so it is your job** | **Not provided, so it is your job** | Included |
| VAT **registration + filing + remittance** | **Yours.** Stripe Tax monitors thresholds and can register on your behalf as a paid option, but the obligation stays with you **[FACT: [Stripe Tax docs](https://docs.stripe.com/tax)]** | **Stripe's**, in 80+ countries **[FACT]** | **Paddle's** **[FACT]** | Provider's | **Yours** | **Yours** | **Polar's**, which holds EU OSS (Ireland), UK and US registrations **[FACT]** |
...
| Suitability for **€1** | Poor (27 %) but *possible*, since the EUR minimum is €0.50 **[FACT]** | Poor (31 %) | **Not offered** at list price | Poor | Poor (~30 %) | Poor (~42 %; micropayments rate helps if approved) | Poor |
...
| Lock-in | Low, because card data is portable on request and the objects are yours | Medium, because the customer relationship sits with Link | **High**, because Paddle owns the customer contract; migrating means re-collecting payment consent | High | Low | Low | Medium-high |
...
- **The German accounting treatment is unresolved.** The MOR fee is invoiced by **Sold
  ...
  **open [ADVISER] questions**, so get written clarification from a Steuerberater **before
...
- **No SEPA Direct Debit** under Managed Payments (cards, wallets, Klarna one-time, plus
  regional methods, but no bank debits) **[FACT: [how it works](https://docs.stripe.com/payments/managed-payments/how-it-works)]**,
  which is a real conversion consideration for German recurring payments. Also: all customer
...
- **Germany-only controlled beta (B2B-only or small B2C):** start on **Stripe direct**. It
  is the cheapest and most controllable, SEPA DD available, and while Kleinunternehmer +
...
foreign VAT and no per-country determination for an MOR to take away, so the premium buys
...
which is acceptable for a coordination service that stores work metadata, less acceptable once
...
- **Costs:** no input-VAT deduction (irrelevant, because the cost base is a €7 VPS), and the
...
- OSS lets you declare all EU B2C VAT in one quarterly return instead of registering in
  each member state. It covers **B2C only**, because B2B reverse-charge sales are not in OSS.
...
- **Umsatzsteuer-Voranmeldung**: monthly if prior-year VAT exceeded **€9,000**; the tax
  ...
  **[FACT: [IHK Düsseldorf](https://www.ihk.de/duesseldorf/existenzgruendung/aktuelles/aussetzung-der-pflicht-zur-monatlichen-uebermittlung-voranmeldungen-in-neugruendungsfaellen-4996474)]**
  Note that **a launch in 2027 may land under the stricter old rule again. [ADVISER]**
...
| Booking the provider's payout as revenue | **You**, typically as one aggregated payout, and **your** supply is to the provider, not to the consumer. Treatment of that supply (and any reverse charge on it) is **[ADVISER]** | **You**, per-transaction |
...
| Chargeback liability | Provider manages disputes, but **not a blanket absorption**. With Stripe MP, standard dispute fees may still apply depending on setup and outcome, and coverage is limited to "eligible card disputes" **[FACT: [MP pricing FAQ](https://support.stripe.com/questions/managed-payments-pricing)]**; exact clawback conditions undocumented **[ADVISER]** | **You** |
...
including taxes, duration, termination conditions, and the withdrawal right. All of it is
...
and the webshop label and the invoice note are **two different obligations with different wording**.
Exact customer-facing wording **[ADVISER]**.

Once VAT-registered (threshold crossed), switch the label to "inkl. MwSt.", and the sticker
...
contract comes into existence**, the customer owes nothing, and payments can be reclaimed.
...
- The EU **Digital Fairness Act** is a Commission initiative announced for **Q4 2026** in
  ...
  **[FACT: [European Parliament Legislative Train](https://www.europarl.europa.eu/legislative-train/theme-protecting-our-democracy-upholding-our-values/file-digital-fairness-act)]**.
  Nothing to comply with yet; **[REC]** design as if it already applied, because the direction of
...
1. Pricing page, with total prices using the §5.2 wording (plain totals + §19 UStG note while
   Kleinunternehmer; "inkl. MwSt." only once VAT-registered), what each tier includes, the numeric limits,
   link to fair-use policy.
2. Checkout summary listing service, term, total price, renewal interval, cancellation terms,
   withdrawal notice, links to AGB and Datenschutzerklärung, **button "zahlungspflichtig
   bestellen"**.
3. Payment step (provider-hosted).
4. Confirmation page.
5. Account → Billing, showing current plan, next renewal date and amount, invoices, and a "cancel"
   entry point.
6. **Public `/kuendigen` page with no login** (§5.5).
7. Cancellation confirmation page showing date/time of the declaration and the end date.
8. **Public `/widerruf` page with no login** (§5.6.1): "Vertrag widerrufen" entry point,
...
- Optional withdrawal-waiver consent **only** if §5.6's waiver path is chosen, using two
...
- **Cancellation confirmation** with the mandatory content per §312k.
- **Withdrawal receipt confirmation**, immediate, on a durable medium, carrying the content of the
  declaration plus date and time of receipt, per §356a.
...
professional developers, teams and agencies, is mostly B2B, yet most of §5 assumes B2C
...
Unternehmer") is **insufficient**, because courts reject AGB-only restrictions. Per OLG Hamm
...
- Plausibility checks: real-time VAT-ID validation is permitted but supplementary only,
  because not every Unternehmer has a USt-IdNr. (Kleinunternehmer customers, for example).
...
can only be served on the free tier (which is fine, since the free tier takes no payment and stays out
of scope), and the B2C surface must still be built before any *public* consumer launch.
Implementation of the restriction is itself **[ADVISER]**, because there is no uniform case law
...
previously omitted, and they are relevant because the app stores project-coordination data created by
...
- **Changes to the product (§327r BGB):** changes beyond maintaining conformity are
  ...
  a notice flow, which dovetails with the price-change notice in §5.8.
...
- **Accessibility (BFSG):** the BFSG covers consumer e-commerce services since
  28 June 2025, but **§3(3) BFSG exempts Kleinstunternehmen providing services**, meaning fewer
...
| **Muster-Widerrufsformular** | Annex to Art. 246a EGBGB | Statutory model form | Only if consumers pay | **Yes** | Statutory template, used verbatim |
| **Widerrufsbutton page (`/widerruf`)** | **§356a BGB** (since 19 June 2026, §5.6.1) | "Vertrag widerrufen" function, form (name, contract ID, contact channel), "Widerruf bestätigen" button, immediate receipt confirmation with date/time | Only if consumers pay | **Yes (B2C)** | Build plus **lawyer review**, since this is a new law with no settled case law yet |
| **B2B-only restriction package** | Only if launch mode 2 of §5.10 is chosen | Prominent notices, checkout confirmation checkbox, mandatory company field, plausibility checks | If paid beta is B2B-only | n/a (superseded by B2C docs at public launch) | **Lawyer review**, because an AGB clause alone fails (OLG Hamm 12 U 52/16) |
...
- **EU-US transfers:** the Commission's EU-US DPF adequacy decision of 10 July 2023 remains
  ...
  **[REC]** Do not treat this as permanent, because Schrems-style litigation risk persists. Keep
...
- **Billing history:** invoices and payment records fall under **§147 AO, 8 years for
  Buchungsbelege and 10 years for books** **[FACT]**. Retention obligations override the
...
- **Entitlement** is the *computed, effective* answer to "what may this account do right
  now?" It is derived, never edited directly. This is what the enforcement layer reads.
- **Plan grant** is a *record* that gives an account a plan without payment. This is what an
  admin creates.

So: "a plan grant is one source of an entitlement; a subscription is the other." Avoid
"comp subscription", "fake subscription" or "free plan" as data-model names, because each of them
...
| `goodwill` | 1–3 months | current plan | Support remedy instead of a refund, which is cheaper than a refund and avoids the fee loss (§2.2) |
...
- No `role = 'friend'` or `role = 'pro'`, because that would put billing state into the
  authorization system and violate the existing single-enforcement-point design.
- No "internal Stripe customer" for permanently free users, and no PII sent to a payment
...
| **Downgrade** | Take effect **at period end**, so no refund is needed, there is no proration argument, and the customer keeps what they paid for. If the new tier's limits are lower than current usage, warn at the moment of downgrade and enforce at the switch |
...
| **Chargeback** | Downgrade immediately, flag the account, block new checkouts, notify the operator. **[REC]** Do not delete data and do not auto-ban the human, because chargebacks are sometimes bank errors |
...
a network call to the provider, because that would put a third party in the critical path of every
API request and violate the "don't couple authentication to the payment provider" rule.

- **Webhooks** are the primary *change feed*, and they mutate local state.
...
- **Reconciliation job [REC]:** hourly, list subscriptions changed since the last run and
  ...
  survivable, and it is not optional infrastructure.
...
| Control | Design |
| --- | --- |
| **Hard limits** | Concurrent sessions, projects, members, active claims, storage, retention. Exceeding returns **HTTP 402 with a machine-readable reason and an upgrade URL**, which agents can parse and report, which fits this product's protocol |
| **Soft limits** | Request rate: throttle (429 + `Retry-After`) rather than refuse. A coordination service that hard-fails on a burst is worse than useless, because the agent will silently stop coordinating |
...
| **Per-user cost tracking** | Cheap proxy metrics recorded per account per day: requests, bytes out, peak concurrent sessions, DB bytes. **[REC]** These double as the §2.6 measurements, so build them *before* pricing is fixed, not after |
...
| **Account sharing** | Detectable via distinct machines and developers per credential (already recorded in `credentials`: `agent`, `machine`, `developer`). **[REC]** Treat sharing as an upsell trigger, not a ban trigger, because the member cap does the real work |
...
## 11.1 Data (conceptual, with no migrations written)
...
- `src/core/plans.ts` holds plan codes, limits and precedence. Pure.
- `src/core/entitlements.ts` holds `computeEntitlement(subscription | null, grants[], userStatus)`.
  **Pure and unit-testable, which is the whole point:** the precedence rules of §9.1 get
  tested without a database or a network.
- `src/server/billing/store.ts` holds the tables above.
- `src/server/billing/provider.ts` is a thin interface (`createCheckoutSession`,
...
- `src/server/billing/webhooks.ts` handles signature verification, idempotent insert and state
  transitions.
- `src/server/billing/reconcile.ts` holds the job from §9.3.
...
| `GET /api/users/me` | **Authenticated only**, because a user must always be able to see their own state, including that they have no plan |
| `GET /api/users`, `PATCH/DELETE /api/users/:id` | **Admin only**, never gated by billing |
| `POST /api/auth/request`, `/redeem`, `GET /api/auth/me` | **Public**, because pairing must work before payment; the entitlement check happens on the project routes |
| `GET /api/auth/pending`, `/credentials`, `DELETE /api/auth/credentials/:id` | **Authenticated**, because credential hygiene must never be paywalled |
| `GET /api/projects` | **Authenticated**; result filtered by entitlement |
| `POST /api/projects/:p/sessions`, `.../heartbeat`, `.../repo` | **Entitlement required**, since this is the metered resource |
| `POST/PATCH /api/projects/:p/claims*`, `/bugs*` | **Entitlement required** |
| `GET /api/projects/:p/state`, `/check` | **Entitlement required**, but **read-only should degrade rather than 402** in `grace`/`expired` for existing projects, because an agent that cannot read state will duplicate work, which is the failure the product exists to prevent |
| `POST /api/billing/checkout`, `GET /api/billing/portal`, `POST /api/billing/cancel` | **Authenticated, never gated**, because you cannot paywall the ability to pay |
| `GET /api/billing/status` | **Authenticated, never gated** |
| `POST /webhooks/:provider` | **Public, signature-verified**, outside `/api/*` entirely |
| `/`, `/AGENT.md`, `/auth.md`, `/install.sh`, `/install/*`, legal pages, `/kuendigen`, `/widerruf` | **Public**, because both cancellation and withdrawal pages must work without login (§5.5, §5.6.1) |
...
1. **Add a verified `billing_email` to `billing_customers`**, because invoices, dunning and the
   ...
   checkout (Stripe) collects the customer's email itself and the webhook delivers it, so
...
2. **Link `credentials` to `users`** (`user_id` FK), because otherwise agent traffic cannot be
   attributed to a paying account.
3. **Fix `cors()` open-to-all** (`src/server/app.ts:68`), because a credentialed billing endpoint
...
6. **Add a pruning job** for retention, because the current schema retains completed claims and
   bugs forever.
...
- **Manual:** everything, with grants by SQL if necessary and approvals by hand.
...
- **Decide first: B2B-only or B2C (§5.10).** This decision defines the whole legal
  ...
  - **Mode 2 (verified B2B-only) [REC]:** no Widerrufsbelehrung, no §312k button, no
    §356a Widerrufsbutton, no §§327ff duties, and instead the B2B restriction package
...
- **Legal (B2C path):** AGB, Widerrufsbelehrung + model form, subscription/cancellation
  ...
  per §5.2, so no "inkl. MwSt."), refund policy. Legal review of AGB, Widerruf and both
  button flows **[REC]**.
...
- **Legal:** everything from Stage 2, reviewed. **If Stage 2 was B2B-only and Stage 3
  ...
  **[REC]** with an explicit country selection plus provider-side restriction, so the €10,000
...
is **Solo at €12/year, annual only**, keeping the same headline price with 4.3 % fees instead of 27 %.
...
Kleinunternehmer wording, which is not "inkl. MwSt."), so the Kleinunternehmer→VAT transition
...
(admin-issued), are resolved by a pure `computeEntitlement()` function with the precedence
...
10. Billing UI: plan page, status page, portal link, **public no-login `/kuendigen` page**
    and, for B2C, a **public no-login `/widerruf` page (§356a)**, kept separate. If the
...
15. Backup and restore tested, because **you are now holding other people's money-backed state**.
...
mode of §5.10 is chosen, since its company field and checkout confirmation are then in scope,
...
5. For EU consumers: **§19a EU-Kleinunternehmerregelung, the €10,000 threshold, or OSS**.
   Which one, and when does the choice have to be made?
...
| **§356a Widerrufsbutton missing (B2C)** | High | Withdrawal period extends up to 12 months + 14 days, plus Abmahnung risk; build the public `/widerruf` page (§5.6.1), because no provider supplies it |
| **B2B-only restriction fails in court** | Medium-high | AGB clause alone is insufficient (OLG Hamm); if the restriction fails, all consumer duties applied retroactively, so implement the full package and have it reviewed |
...
| **MOR lock-in** | Medium | Thin provider interface; the customer relationship sits with the MOR, which is a real cost of the recommendation |
...
> Launch invited friends using permanent or time-limited Studio **plan grants**, with no
> payments and no provider. **Instrument real usage before fixing public limits**; every
